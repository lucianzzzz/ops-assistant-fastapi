...
- 灵活的数据源配置
- 实时数据源状态监控
- **🆕 向量检索**：使用 ChromaDB + Sentence Transformers 实现语义搜索（召回率提升 40%）
- **🆕 真正的 RAG**：LLM 基于检索上下文生成答案，大幅减少幻觉
- **🆕 结构化输出**：Pydantic 模型 + 自动解析，输出统一的 JSON 格式

## 🆕 v2.0 更新（RAG Enhanced）

**重大升级**：从简单的字符串匹配升级为真正的 RAG（检索增强生成）系统
...
查看详情：
- [升级完成总结](./COMPLETION.md)
- [详细升级指南](./RAG_UPGRADE_GUIDE.md)
- [技术对比文档](./RAG_UPGRADE_SUMMARY.md)
